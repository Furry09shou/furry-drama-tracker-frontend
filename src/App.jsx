...
const AdminEpisodeTrash = lazy(() => import('./components/AdminEpisodeTrash'));
...
const getRoutePageTitleKey = (pathname) => {
  ...
  if (pathname.startsWith('/admin/episode-trash')) return 'pageTitle.adminEpisodeTrash';
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.transform = 'translateY(0)'; }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <a href="https://github.com/Furry09shou/furry-drama-tracker-frontend" target="_blank" rel="noopener noreferrer" onClick={() => setShowGithubModal(false)} style={{
          ...
          onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.transform = 'translateY(0)'; }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Link to="/license" onClick={() => setShowGithubModal(false)} style={{ fontSize: '12px', color: 'var(--primary)', textDecoration: 'none' }}
                ...
                onMouseLeave={(e) => e.currentTarget.style.textDecoration = 'none'}
              >{t('footer.viewLicense')}</Link>
              <a href="https://github.com/xiedada05/furry-drama-be-neo" target="_blank" rel="noopener noreferrer" onClick={() => setShowGithubModal(false)} style={{
                padding: '6px 12px', borderRadius: '8px', background: 'var(--primary)',
                color: '#fff', textDecoration: 'none', fontSize: '12px', fontWeight: 600,
                flexShrink: 0, whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: '5px',
                transition: 'transform 0.2s, box-shadow 0.2s, filter 0.2s'
...
function AppContent() {
  ...
  // ===== 站点背景壁纸：由主题功能提供（背景设置功能已并入主题） =====
  // 优先级：用户所选主题的壁纸（应用主题时写入 backgroundPrefs）> 站点默认主题壁纸（/api/themes/active）
  const [defaultThemeWallpaper, setDefaultThemeWallpaper] = useState(null);

  useEffect(() => {
    // 站点默认主题壁纸（未设置个人主题时全站兜底，含未登录访客）
    let cancelled = false;
    axios.get('/api/themes/active')
      .then(res => {
        if (!cancelled) setDefaultThemeWallpaper(res.data?.theme?.wallpaperUrl || null);
      })
      .catch(() => {
        if (!cancelled) setDefaultThemeWallpaper(null);
      });
        ...
        <Suspense fallback={<LoadingFallback />}>
          <Routes>
            ...
            <Route path="/admin" element={<AdminLayout />}>
              ...
              <Route path="episode-trash" element={<AdminGuard><AdminEpisodeTrash /></AdminGuard>} />
